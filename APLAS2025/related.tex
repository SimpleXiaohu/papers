...
prior approaches %to string constraint solving 
are classified into three main categories: automata-based (relying on finite automata to represent the domain of string variables
...
%Generally speaking, 
From another perspective, there are two lines of work aiming for building practical string constraint solvers. %could essentially be classified into two categories. 
(1) One could support as many operations as possible, but primarily resort to heuristics, offering no completeness/termination guarantees. 
...
Many string constraint solvers are implemented in SMT solvers, %, such as cvc5 [9] or Z3 [31], 
...
including Norn \cite{Abdulla14}, SLOTH \cite{HJLRV18} and
