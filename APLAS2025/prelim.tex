...
%
Intuitively, we remove the string equalities of the form 
$y = f(x_1, \myvec{it_1}, \cdots, x_l, \myvec{it_l})$ (where $\myvec{it_1}$, $\cdots$, $\myvec{it_l}$ are vectors of integer terms)
% $y = f(x_1, \cdots, x_l, it_1,\cdots, it_k)$ (where $it_1$, $\cdots$, $it_k$ are integer terms)
one by one by computing the pre-images and adding the resulting CEFA membership constraints for $x_1, \cdots, x_l$. In the end,  the original string constraint is transformed into a conjunction of CEFA membership constraints and LIA formulas, whose satisfiability checking %can be solved in 
