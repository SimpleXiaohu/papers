...
It turns out that to solve the nonemptiness of CEFA in $\Lambda'_{x_1}, \cdots, \Lambda'_{x_n}$ w.r.t. $\varphi$, we can ignore the characters in $\cB_1,\cdots, \cB_n$. This is due to the fact that there is only one CEFA for each string variable and $\varphi$ only cares about the values of registers.
