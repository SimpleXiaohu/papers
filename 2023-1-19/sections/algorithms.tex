...
the $\myvec{0}$-labeled transitions in $\cU_{\cB_i}$ do not contribute to the final output $\sum \limits_{\myvec{v} \in \costset(\cB_i)} \eta(\myvec{v}) \myvec{v}$. Regardless of alphabet, we take register updates as new alphabet symbols and $\myvec{0}$ as $\epsilon$ symbol. Then we are capable of using standard minimization to reduce the automata size.
