...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    \Require The CEFAs $auts$, max string length $bound$, other LIA formula $\psi$
    ...
    \While {$bound \geq 0$}
      \For{$\aut \in auts$}
        \For{$(q, \myvec{V}) \gets lastStates(\aut)$}
          \If{$q\in F$}
            \State $\varphi_{\aut} \gets \varphi_{\aut} \bigvee\limits_{i\in[1,m]} r_i = \myvec{V}[i])$
          \EndIf
