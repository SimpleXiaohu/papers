...
%
\paragraph*{Step 1.} For each $i \in [n]$, compute a CEFA $\cB_{i} = \aut_{i,1} \cap \cdots \cap \aut_{i, l_i}$, and let $\Lambda'_{x_i} := \{\cB_i\}$. \qed
...
Let $\aut = (R, Q, \Sigma, \delta, I, F)$ be a CEFA and $\costset_\aut = \{\myvec{v} \mid \exists q, a, q'.\ (q, a, q', \myvec{v}) \in \delta\}$. 
Moreover, let $\aut'$ be an NFA over the alphabet $\costset_\aut$ that is obtained from $\aut$ by ignoring the characters, that is, the transitions of $\NFA'$ are the tuples $(q, \myvec{v}, q')$ such that $(q, a, q', \myvec{v}) \in \delta$ for some $a \in \Sigma$.
%Then $\cefaout(\aut) = \parikh(\NFA')$.
\begin{observation}\label{obs-cefa-nfa}
For each CEFA $\aut$, $\cefaout(\aut) = \left\{\sum \limits_{\myvec{v} \in \costset_\aut} \eta(\myvec{v}) \myvec{v}  \mid \eta \in \parikh(\NFA') \right\}$.
\end{observation}



From Observation~\ref{obs-cefa-nfa}, we know that the following two conditions are equivalent, 
\begin{itemize}
%\item CEFA in $\Lambda'_{x_1} = \{\cB_1\}, \cdots, \Lambda'_{x_n} = \{\cB_n\}$ are nonempty w.r.t. $\varphi$, namely, 
%there are an assignment $\theta: \{x_1, \cdots, x_n\} \rightarrow \Sigma^*$ and vectors $\myvec{v_{1}}, \cdots, \myvec{v_{n}}$ such that $(\theta(x_1); \myvec{v_1}) \in \Lang(\cB_1)$, $\cdots$, $(\theta(x_n); \myvec{v_n}) \in \Lang(\cB_n)$ and $\varphi[\myvec{v_{1}}/R_{\cB_{1}}, \cdots, \myvec{v_{n}}/R_{\cB_{n}}]$ is true,
%
...
%
\item 
there are $\eta_1 \in \parikh(\cB'_1), \cdots, \eta_n \in \parikh(\cB'_n)$ such that 
%
%\begin{equation}\label{eqn-cefa-lia}
\[
\varphi \left[\sum \limits_{\myvec{v} \in \costset_{\cB_1}} \eta(\myvec{v}) \myvec{v} \big{/}R_{\cB_1}, \cdots, \sum \limits_{\myvec{v} \in \costset_{\cB_n}} \eta(\myvec{v}) \myvec{v} \big{/}R_{\cB_n} \right]
\]
%\end{equation} 
%
is true. 
...
Therefore, to solve the nonemptiness of CEFA in $\Lambda'_{x_1}, \cdots, \Lambda'_{x_n}$ w.r.t. $\varphi$, it is sufficient to compute the existential LIA formulas $\psi_{\cB'_1}(\fZ_{\costset_{\cB_1}}), \cdots, \psi_{\cB'_n}(\fZ_{\costset_{\cB_n}})$, which define the Parikh images of $\cB'_1$, $\cdots$, $\cB'_n$ respectively, and solve the satisfiability of the following existential LIA formula
\begin{equation}\label{eqn-cefa-lia}
\bigwedge \limits_{i \in [n]} \psi_{\cB'_i}(\fZ_{\costset_{\cB_i}}) \wedge 
...
Because the sizes of the LIA formulas $\psi_{\cB'_1}(\fZ_{\costset_{\cB_1}}), \cdots, \psi_{\cB'_n}(\fZ_{\costset_{\cB_n}})$ are proportional to the sizes (more precisely, the alphabet size, the number of states and transitions) of NFA $\cB'_1, \cdots, \cB'_n$, and it is expensive (NP-complete) to solve the satisfiability of existential LIA formulas, it is vital to reduce the sizes of these NFA.

We also have the following observation: For each $i \in [n]$ and $\eta \in \parikh(\cB'_i)$, we have 
...
In other words, the $\myvec{0}$-labeled transitions in $\cB'_i$ do not contribute to the sum $\sum \limits_{\myvec{v} \in \costset(\cB_i)} \eta(\myvec{v}) \myvec{v}$.  

Therefore, we can reduce the sizes of $\cB'_1, \cdots, \cB'_n$ by Step II presented in the sequel. 

\paragraph*{Step 2.} For each $i \in [n]$, view the transitions $(q, \myvec{0}, q')$ in $\cB'_i$ as $\varepsilon$-transitions $(q, \varepsilon, q')$, then remove the $\varepsilon$-transitions from $\cB'_i$, determinize and minimize the resulting NFA.  \qed

For $i \in [n]$, let  $\cC_i$ denote the DFA obtained from $\cB'_i$ by executing Step 2. From the construction, we know that $\parikh(\cC_i) = \parikh(\cB'_i)$ for each $i \in [n]$.
Therefore, we compute LIA formulas from $\cC_i$'s, instead of $\cB'_i$'s, to represent Parikh images. 


\paragraph*{Step 3.} For each $i \in [n]$, compute an existential LIA formula $\psi_{\cC_i}(\fZ_{\costset_{\cB_i}})$ from $\cC_i$ to represent $\parikh(\cC_i)$. Then solve the satisfiability of the following existential LIA formula 
%
\[
...
Finally, although we propose techniques to reduce the sizes of $\cB'_i$'s, it is still relatively expensive to compute LIA formulas from NFA and solve their satisfiability, especially for those problem instances with positive answers, namely, those instances where CEFA in $\Lambda_{x_1}, \cdots, \Lambda_{x_n}$ are nonempty w.r.t. $\varphi$.   
Therefore, in the sequel, we propose a heuristic to search for solutions for those positive problem instances by utilizing under approximations, before Step 3. Intuitively, we search for solutions by bounding the lengths of the strings that are accepted by $\cC_i$'s. The heuristic uses a constant bound $B > 0$. Since the heuristic is applied before Step 3, we call it Step 2.5, to denote the fact that it is applied after Step 2 and before Step 3. 

For $i \in [n]$, suppose $R_{\cB_i} = (r_{i,1},\cdots, r_{i, k_i})$ and $\cC_i = (Q'_i, \costset_{\cB_i}, \delta'_i, q_{i,0}, F'_i)$. 

\paragraph*{Step 2.5.} Initially, let $len := 0$, and for each $i \in [n]$, let $\confset_{\cC_i}:=\{(q_{i,0}, \myvec{0})\}$ and $\psi_{\cC_i}:= \lfalse$.  Then iterate the following procedure until $len = B$.
\begin{quote}
For each $i \in [n]$, let $\confset'_{\cC_i}:=\confset_{\cC_i}$ and do the following:  For each $(q, \myvec{v}) \in \confset_{\cC_i}$,
\begin{itemize}
...
%
\item for each transition $(q, \myvec{v'}, q') \in \delta'_i$, let $\confset'_{\cC_i}:=\confset'_{\cC_i} \cup \{(q', \myvec{v}+ \myvec{v'})\}$. 
\end{itemize}
Let $\varphi:=\varphi \wedge \psi_{\cC_i}$. 

If $\varphi$ is satisfiable, then return $\ltrue$. Let $len: = len+1$. \qed
\end{quote}
