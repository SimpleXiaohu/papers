...
\denghang{The encoding of counting need to be corrected}\newline
For a handled regular expression $\regex\{m,n\}$, suppose the CEFA recognizing $\regex$ is $\aut = (Q,\Sigma,\delta,q_I,F,\emptyset,\top)$, then the CEFA recognizing $\regex\{m,n\}$ is defined as $\aut_{m,n} = (Q, \Sigma, \delta',q_I, F\cup\{q_I\}, r, \theta)$ where:
\begin{itemize}
...
\end{itemize}
The construction is illustrated in Fig.\ref{fig:construct_repetition}, where a new register $r$ is added to store the counting times. The transition $q\xrightarrow[(1)]{\epsilon} q_I$ is used to update the counting times for each $q\in F$. The accepting condition $\theta'$ constrains the counting times of the accepting word to the range $[m, n]$. After the construction, we need to eliminate the transition with epsilon character. The elimination is done after adding transition $q\xrightarrow[(1)]{a}$  \newline
\begin{figure}[h]
...
\begin{example}
  ...
  \begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
      ...
      \end{tikzpicture}
      \caption{The CEFA recognizing $c$}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
      ...
      \end{tikzpicture}
      \caption{The CEFA recognizing $\Sigma \setminus a$}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        (q_1) edge [bend right] node [left] {$\scriptstyle \epsilon:(\myemph{1})$} (q_0)
        (q_0) edge           node [right] {$\scriptstyle c:(0)$} (q_2)
        (q_2) edge [bend left] node [left] {$\scriptstyle \epsilon:(\myemph{1})$} (q_0);
      \end{tikzpicture}
...
Given a string-length constraint, we build a CEFA with a register storing the length of accepted word. Namely, the CEFA of length constraint is $(\{q_I\}, \Sigma, \{q_I\xrightarrow[(1)]{\Sigma} q_I\}, q_I, \{q_I\}, r, \theta)$ where accpeting condition $\theta$ is based on the length value. For example, example \ref{eg:pre_len} shows the CEFA of length constraints $|x| > 120$, in which the corresponding $\theta(q_I)$ is $r > 120$. This CEFA contains a simple loop which increase $1$ on register $r$ for all char in $\Sigma$. The value of $r$ (i.e., the length of word) after an accepting run shoud greater than $120$, so the language of the CEFA is the set of all words whose length is greater than 120.  \newline
...
\begin{example}[The CEFA of $|x| > 120$]\label{eg:pre_len} 
  \begin{figure}[h]
    ...
    \begin{tikzpicture}[
      shorten >=1pt,node distance=2cm,on grid,>={Stealth[round]},
      initial text=, every state/.style={minimum size = 0.001cm},
      accepting text=$\scriptstyle{r > 120}$, accepting/.style=accepting by arrow
      ]
...
After encoding counting operator and length constraint to CEFAs, there are only automaton memberships $x\in \aut$ and linear integer arithmetic $t_1 \ \op \ t_2$ where $\aut$ is CEFA and $t_1, t_2$ are integer terms. Let $\psi$ denotes the conjunction of all LIA formulas $t_1\ \op\ t_2$ and $\Lambda_x = \{\aut_x^1, \aut_x^2,\cdots \aut_x^n\}$ denotes the set of CEFAs in the automaton membership $x\in \aut$. The emptiness checking problem of $\Lambda_x$ under linear integer arithmetic $\psi$ is to check whether there exists a word $w$ such that $w\in \aut_x^1, w\in \aut_x^2\cdots w\in \aut_x^n$ and $\psi[\myvec{V}/\myvec{r}]$ holds, where $\myvec{r}$ are all registers in automata of $\Lambda_x$ and $\myvec{V}$ are corresponding register values after running $w$. The emptiness problem is \emph{abbreviated as $SAT_{CEFA}[LIA]$ problem}. \newline
...
The main idea of our framework is to simplify the input CEFAs firstly and under-approximate the solution of the \emptinessprob{} by adding length bounds on string variables. If the under-approximation is satisfiable, the CEFAs is not empty under $\psi$. Otherwise, we check the Parikh image of the simplified CEFAs. If the Parikh image is satisfiable, the CEFAs under linear integer arithmetic $\psi$ are not empty. Or else, they are empty. The framework is \emph{sound} and \emph{complete}.\newline
The framework is shown in Algorithm \ref{alg:emptiness}, we simplify the input CEFAs at line 1 because they may have many transitions and registers. The main idea of the simplification is treating alphabet as unary. The symbolic updates of the CEFAs are maintained, which are then treated as labels of transitions in the minimization. After the simplification, we try to find a solution by under-approximation at line 3 and line 4. An off-the-shelf SMT solver gives the result of LIA $\varphi_{under}\wedge \psi$. If the LIA is satisfiable, we return $true$ at line 5 because it implies that the CEFAs under the LIA $\psi$ are not empty. Otherwise, we compute the Parikh images of the simplified CEFAs and check if the Parikh images are satisfiable in conjunction with $\psi$ at line 12. The Parikh image checking is complete so that satisfiability implies non-emptiness directly. \newline
The incremental under-approximation program is executed many times until the $MaxBound$ is reached. $MaxBound$ is an empirical bound set to $15$ in our experiment because it brings exponential growth of searching space. The details of simplification are shown in Algorithm \ref{alg:simplify} and under-approximation are shown in Algorithm \ref{alg:underApprox}.\newline
\begin{algorithm}[h]
...
The purpose of simplification is to remove duplicated transitions and registers. In the emptiness checking Algorithm \ref{alg:emptiness}, the non-emptiness is equal to the reachability of CEFAs. Thus, the symbolic updates on transitions are meritorious because it influences registers values, while the characters are not. The alphabet of the CEFA can be seen as unary (i.e., the alphabet is $\{a\}$) and the vectors as labels. The special vector $\myvec{0}_n$ with only zero elements is seen as $\epsilon$ label. Namely, given a CEFA $\aut = (Q, \Sigma, \delta, q_I, F, R, \theta)$, we obtain a symbolic form  $\aut_{sym} = (Q, \Sigma', \delta', q_I, F, R, \theta)$ where
\begin{itemize}
  \item $\Sigma' = \{a\}$,
...
\end{algorithm}
\begin{example}
  Considering the CEFA (Fig.\ref{subfig:count_aut_sigma_minus_a_5_60}) recognizing $(\Sigma\setminus a)\{5,60\}$ where $\Sigma = \{a,b,c\}$. We first obtain a symbolic form of it by using unary alphabet (Fig.\ref{subfig:simp_symbolic_cefa}) and then minimize the symbolic form based on the label "$a:(1)$"(Fig.\ref{subfig:simp_final}).
  \begin{figure}[h]
    \begin{subfigure}[b]{0.49\textwidth}
      \centering
      \begin{tikzpicture}[
        shorten >=1pt,node distance=1cm,on grid,>={Stealth[round]},
        initial text=, every state/.style={minimum size = 0.001cm},
        accepting text=$\scriptstyle 5\leq r \leq 60$, accepting/.style=accepting by arrow,
        ]
        ...
        \path[->] (q_0) edge   node    [right] {$\scriptstyle \myemph{a:(1)}$} (q_1)
        (q_0) edge   node    [right] {$\scriptstyle \myemph{a:(1)}$} (q_2)
        (q_1) edge[bend right]   node    [left] {$\scriptstyle \myemph{a:(1)}$} (q_0)
        (q_2) edge[bend left]   node    [left] {$\scriptstyle \myemph{a:(1)}$} (q_0);
      \end{tikzpicture}
      \caption{The symbolic form of the CEFA in Fig.\ref{subfig:count_aut_sigma_minus_a_5_60}}
    ...
    \begin{subfigure}[b]{0.49\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[state,initial]            (q_0)                      {};
        \node[state, accepting]         (q_1) [right=of q_0]       {};
        \path[->] (q_0) edge   node    [above] {$\scriptstyle\myemph{a:(1)}$} (q_1)
        (q_1) edge[loop below] node {$\scriptstyle\myemph{a:(1)}$} (q_1);
      \end{tikzpicture}
