...
For convenience, we use $\varphi(\vec{x})$ with $\myvec{x} = (x_1, \cdots, x_k)$ to denote an LIA formula $\varphi$ whose free variables are from $\{x_1, \cdots, x_k\}$. For an LIA formula $\varphi(\vec{x})$ with $\myvec{x} = (x_1, \cdots, x_k)$ and $\myvec{t} = (t_1, \cdots, t_k)$ be a tuple of integer terms, we use $\varphi[\myvec{t}/\myvec{x}]$ to denote the formula obtained by replacing (simultaneously) $x_i$ with $t_i$ for every $i \in [k]$.
