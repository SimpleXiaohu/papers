...
Assume that $u=a_1\cdots a_{n}$ is nonempty and $1\leq i<j \leq n$. %Then a \emph{position} of $u$ is a number $i \in [|u|]$ (Note that the first position is $1$, instead of  0). In addition, 
...
A \emph{(nondeterministic) finite automaton} (NFA)  is a tuple $\NFA=(Q, \Sigma, \delta, I, F)$, where $Q$ is a finite set of states, $\Sigma$ is a finite alphabet, $\delta \subseteq Q \times \Sigma \times Q$ is the transition relation, $I \subseteq Q$ is the set of initial states, and $F \subseteq Q$ is the set of final states. For readability, we write a transition $(q, a, q') \in \delta$ as $q \xrightarrow[\delta]{a} q'$ (or simply $q \xrightarrow{a} q'$). \denghang{Note that one initial state is sufficient. A set of initial states is a technical choice to simplify the defination of union operation in Sec \ref{sec:automaton}.}%Moreover, when $\delta$ is clear from context, we omit $\delta$ in $q \xrightarrow[\delta]{a} q'$ and write $q \xrightarrow{a} q'$. 
...
Moreover, the class of regular languages is closed under union, intersection, concatenation, Kleene star, complement, and language difference \cite{HU79}. \sout{The constructions of NFA corresponding to these language operations are list in \ref{appendix:cefa}}
% In the sequel, we recall the constructions of NFA corresponding to these language operations. 
...
%\tl{to put in a different place later} Note that our decision procedure also applies to general transducers. However, the EXPSPACE complexity bound is not because the distributive property $f^{-1}(L_1\cap L_2)= f^{-1}(L_1)\cap f^{-1}(L_2)$ for regular languages $L_1, L_2$ only holds for functional transducers $f$.  

%We remark that an FT usually defines a relation.

%\smallskip

%In this paper, we consider logic involving two data types, i.e., the string and integer data types. We will use $u, v, \dots$ to denote string constants,  $c, d,\dots$ to denote integer constants, $x, y, \dots$ to denote string variables, and $i, j, \dots$ to denote integer variables.
...
%For convenience, we assume a set $X$ of variables is a vector. 
