...
Finally, The satisfiability of the original string constraint is reduced to the nonemptiness of the CEFA $\aut\equiv \aut_1 \cap \aut_2 \cap \aut_3$ with respect to the LIA formula $\varphi \equiv r_4 > 120$, abbreviated as $\sf NE_{\varphi}(\aut)$. That is, whether there exist $w \in \Sigma^*$ and $(v_1, v_2, v_3, v_4) \in \Int^4$ such that $w$ is accepted by $\aut$, producing resulting registers values $(v_1, v_2, v_3, v_4)$ that satisfy $1 \le v_1 \le 60\wedge 1 \le v_2 \le 60 \wedge 0 \le v_3 \le 60$ and $\varphi$. 
It is not hard to observe that $\sf NE_{\varphi}(\aut)$ is independent of the characters of $\aut$.  Therefore, the characters in $\aut$ can be ignored, resulting into an NFA $\cB$ over the alphabet $\costset$, where $\costset$ is the set of vectors from $\Int^4$ occurring in the transitions of $\aut$ (see Figure~\ref{fig:overview:product:reduced}(a)). Then $\sf NE_{\varphi}(\aut)$ is reduced to the problem of deciding whether there exists $w' \in \costset^*$ such that it is accepted by $\cB$ and its Parikh image (i.e., numbers of occurrences of characters), say $\eta_{w'}: \costset \rightarrow \Nat$, satisfies that $1 \le v'_1 \le 60\wedge 1 \le v'_2 \le 60 \wedge 0 \le v'_3 \le 60$ and $v'_4 > 120$, where $(v'_1, v'_2, v'_3, v'_4) =  \sum \limits_{\myvec{v} \in \costset} \eta_{w'}(\myvec{v}) \myvec{v}$ for each $\myvec{v}\in \costset$. Intuitively, $(v'_1, v'_2, v'_3, v'_4)$ is a weighted sum of vectors $\myvec{v} \in \costset$, where the weight is the number of occurrences of $\myvec{v}$ in $w'$. (See Section~\ref{subsec:cefadec} for more detailed arguments.)
...
Nevertheless, the algorithm illustrated above is insufficient when regexes are complicated. The size of $\cB$ can still be big, causing the resulted existential LIA formulas are of big sizes and hard to be solved (NP-complete \cite{Haase18}). 
To tackle this issue, we propose techniques to reduce the sizes of $\cB$ over the alphabet $\costset$.
...
Specifically, to reduce the sizes of the $\cB$, we determinize $\cB$ and apply the minimization algorithm to the resulting deterministic finite automaton (DFA), obtaining a DFA $\cC$, as illustrated in Figure~\ref{fig:overview:product:reduced}(b).
...
The evaluation in section \cite{sec} shows that we can solve 1503 more instances after utilizing simplification techniques, with \%54.8 less average time.

We implement all the aforementioned techniques on the top of OSTRICH, resulting in a solver $\ostrichrecl$. It turns out that $\ostrichrecl$ is able to solve the string constraint in~(\ref{eqn-running}) within one second, while the state-of-the-art string solvers are incapable of solving it within 300 seconds. 
