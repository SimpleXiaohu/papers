...
Specifically, to reduce the sizes of $\cB$, we take $\vec{0}$-labeled transitions as $\epsilon$-transitions, determinize $\cB$, and apply the minimization algorithm to the resulting deterministic finite automaton (DFA), resulting in a DFA $\cC$, as illustrated in Figure~\ref{fig:overview:product:reduced}(b).
