...
Although the determinization and minimization may not reduce the sizes of NFA in the worst case, the experiment results show us that they do reduce the sizes of NFA constructed from the regular expressions in practice. (See Table~\ref{tab:results_heuristics}, where adding the size-reduction techniques reduces the running time for 46.4\%.) 
...
We implement all the aforementioned techniques on top of OSTRICH, resulting into a solver $\ostrichrecl$. It turns out that $\ostrichrecl$ is able to solve the string constraint in~(\ref{eqn-running}) within one second, while the state-of-the-art strings solvers cannot solve it within 300 seconds. 
