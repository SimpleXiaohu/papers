...
The \emph{union} of $\aut_1$ and $\aut_2$ is denoted by $\aut_1 \cup \aut_2$. Two fresh auxiliary registers say $r'_1,r'_2 \not \in R_1 \cup R_2$, are introduced so that the accepting condition knows whether a run is from $\aut_1$ (or $\aut_2$).\hdh{This is mainly because the same accepting condition is applied to all final states, in other words, the accepting condition does not distinguish between final states.} Specifically, $\aut_1 \cup \aut_2 = (R', Q', \Sigma, \delta', I', F', \alpha')$ where 
\begin{itemize}
