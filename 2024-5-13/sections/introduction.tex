...
\begin{itemize}
  \item We propose an automata-theoretical approach for solving RECL constraints. \zhilin{update this paragraph later}
...
Section~\ref{sec:heuristic} presents some heuristics for improving the performance of solving RECL constraints where the counting operators are inside the scope of some other counting operators or complement operators. \zhilin{double check later}
Section~\ref{sec:implementation} shows the experiment results. Section~\ref{sec:conclu} concludes this paper.

A preliminary version of this paper has appeared in the proceedings of SETTA 2023~\cite{Denghang2023}. This paper extends the SETTA paper in the following two aspects: (i) To improve the performance of complex RECL constraints where the counting operators are nested with some other counting operators or nested with the complement operators, various heuristics are proposed, (ii) moreover, to evaluate the performance of these heuristics, the benchmarks as well as the experiments are extended.   \zhilin{double check later}
