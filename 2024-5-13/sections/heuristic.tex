...
%
Let $\ufld_1(e)$ and $\ufld_2(e)$ denote the resulting regular expressions. \hdh{Note that in $\ufld_1(e)$, all the counting operators in $e_1$ (if there is any) disappear, while in $\ufld_2(e)$, all the counting operators in $e_1$ are preserved and copied for $n$ times. }
...
%to make a decision whether to choose $ufld_1(e)$ or $ufld_2(e)$. 

\item To estimate the sizes of $\cU_{\aut_{\ufld_1(e)}}$ and $\cU_{\aut_{\ufld_2(e)}}$, we first estimate the sizes of the  CEFAs $\aut_{\ufld_1(e)}$ and $\aut_{\ufld_2(e)}$   
as \hdh{two pairs $(\snum_1(e), \anum_1(e))$ and $(\snum_2(e),\anum_2(e))$, where $\snum_1(e)$ and $\anum_1(e)$ are the estimates of the number of states and integer vectors in $\aut_{\ufld_1(e)}$ separately, similarly for $(\snum_2(e)$, $\anum_2(e))$. Since the NFAs $\cU_{\aut_{\ufld_1(e)}}$ and $\cU_{\aut_{\ufld_2(e)}}$ are constructed from $\aut_{\ufld_1(e)}$ and $\aut_{\ufld_2(e)}$ by dropping the accepting conditions and ignore characters, $\snum_1(e)$ and $\anum_1(e)$ are the estimates of the number of states and characters of $\cU_{\aut_{\ufld_1(e)}}$, similarly for $\cU_{\aut_{\ufld_2(e)}}$. Then we estimate the sizes (number of transitions) of the NFAs $\cU_{\aut_{\ufld_1(e)}}$ and $\cU_{\aut_{\ufld_2(e)}}$ as $sz_1 = (\snum_1(e))^2 \cdot \anum_1(e)$ and $sz_2 = (\snum_2(e))^2 \cdot \anum_2(e)$ respectively.}
%
\mycomment {\item From the construction in Section~\ref{sec:algorithm}, 
two NFAs $\cU_{\aut_{\ufld_\hdh{1}(e)}}, \cU_{\aut_{\ufld_2(e)}}$ are constructed from $\aut_{\ufld_1(e)}$ and $\aut_{\ufld_2(e)}$ by dropping the accepting conditions and ignoring the characters. As a result, the alphabets of $\cU_{\aut_{\ufld_\hdh{1}(e)}}, \cU_{\aut_{\ufld_2(e)}}$ are the set of integer vectors occurring in the transitions of $\aut_{\ufld_1(e)}, \aut_{\ufld_2(e)}$ respectively. \hdh{Note that there is only one register and two integer vectors in $\aut_{\ufld_1(e)}$, in other words, the number of registers and the alphabet size are fixed. }
...
\hdh{It remains to show how to compute $(\snum_1(e), \anum_1(e))$ and $(\snum_2(e),$ $ \anum_2(e))$ from $e=(e_1)^{\{m,n\}}$. Let $e=(e_1)^{\{m,n\}}$ be a regular expression where $e_1$ is a register-representable regular expression. Note that there is only one register in $\aut_{\ufld_1(e)}$. As a result, we set $\anum_1(e) = 2$. Moreover, in the sequel, we inductively compute $\snum'_1(e_1)$ and $(\snum'_2(e_1), \anum'_2(e_1))$ from $e_1$, then let $\snum_1(e)  = \snum'_1(e_1)$ and $(\snum_2(e), \anum_2(e)) = (\snum'_2(e_1),$ $ \anum'_2(e_1))$. 
\begin{itemize}
  \item If $e_1 = \emptyset$ or $e_1 = \epsilon$, then $\snum'_1(e_1)  = 0$ and 
  $$
  \begin{array}{l}
    (\snum'_2(e_1), \anum'_2(e_1))  = (0, 0).
  \end{array}
  $$
  (Note that here the constant $0$, instead of $1$, is chosen, so that later on, when we consider $e_1 = e_2 \cap e_3$, $\snum'_2(e_2) \times \snum'_2(e_3)$ and $\vnum'_2(e_2) \times \vnum'_2(e_3)$ will become 0 since $\epsilon\cap e_3 = \epsilon$ and $\emptyset\cap e_3 = \emptyset$)
%
  \item If $e_1 = a$, then $\snum'_1(e_1) = 2$ and 
  $$
  \begin{array}{l}
    (\snum'_2(e_1), \anum'_2(e_1)) = (2n, 1).
  \end{array}
  $$
 %
...
%
 ...
 \ \ \ \ (  n(\snum'_2(e_2) + \snum'_2(e_3)), n(\anum'_2(e_2) + \anum'_2(e_3))).
 \end{array}
  $$
  Note that because $e_1$ is concatenated for $n$ times in $\ufld_2(e)$, we define $\snum'_2(e_1)$ as  $n(\snum'_2(e_2) + \snum'_2(e_3))$. Moreover, since the integer vectors in $\aut_1 \cdot \aut_2$ are of the form $(\myvec{v}_1,\myvec{0})$ and $(\myvec{0}, \myvec{v}_2)$ where $\myvec{v}_1$ and $\myvec{v}_2$ are integer vectors in $\aut_1$ and $\aut_2$ respectively, we define $\anum'_2(e_1)$ as $n(\anum'_2(e_2) + \anum'_2(e_3)))$.
%
...
%
  ...
  \begin{array}{l }
 (\snum'_2(e_1), \anum'_2(e_1)) =  \\
  \ \ \ \ (n(\snum'_2(e_2) + \snum'_2(e_3)+1),   n(2\anum'_2(e_2) + 2\anum'_2(e_3))).
 \end{array}
  $$
...
\hdh{\paragraph*{Combination of under- and over-approximation}
 We combine the under- and over-approximation as well as the decision procedure into the following procedure.  
 \begin{enumerate}
 \item We first apply the under-approximation.
 \item If the under-approximation returns ``sat'', then we know that the original constraint is satisfiable. Otherwise, the under-approximation returns ``unsat'' and we apply the over-approximation.
 \item If the over-approximation returns ``unsat'', then we know that the original constraint is unsatisfiable. Otherwise, the over-approximation returns ``sat'' and we resort to the decision procedure (without approximations) in Section~\ref{sec:algorithm} to solve the RECL constraint. 
 \end{enumerate}
% In our configuration, under-approximation is performed first due to its superior computational efficiency, and we proceed with over-approximation if under-approximation yields an "unsat" result. If the over-approximation yields a "sat" result, we employ the decision procedure to obtain an answer, which implies that our approach is sound and complete. To minimize computational overhead, we apply approximation techniques only in cases involving non-register-representable regular expressions with significant counting bounds—specifically, those exceeding 20 in our experimental configuration.
}
