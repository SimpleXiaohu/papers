...
\hdh{\paragraph*{Combination of under- and over-approximation}
 In our configuration, under-approximation is performed first due to its superior computational efficiency, and we proceed with over-approximation if it yields an unsatisfiable result. In cases where over-approximation produces a satisfiable result, we execute the decision procedure wherein all non-register-representable regular expressions are naively unfold. To minimize computational overhead, we apply approximation techniques only in cases involving non-register-representable regular expressions with significant counting bounds—specifically, those exceeding 20 in our experimental configuration.
}
