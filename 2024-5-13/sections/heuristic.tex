...
It remains to show how to compute the pairs $(\snum_1(e), \rnum_1(e))$ and $(\snum_2(e), \rnum_2(e))$ from $e=(e_1)^{\{m,n\}}$. Let $e=(e_1)^{\{m,n\}}$ be a regular expression where $e_1$ is a register-representable regular expression. Then we compute $\snum'_1(e_1)$ and $(\snum'_2(e_1), \rnum'_2(e_1))$\hdh{(TODO:Change all the $num'$ to $num$, to avoid ambiguity)} by a structural induction on $e_1$, \hdh{in condition, $\rnum_1(e_1)$ is set to $1$.} \sout{Finally, let $(\snum_1(e), \rnum_1(e)) = (\snum'_1(e_1), 1)$ and $(\snum_2(e), \rnum_2(e)) = (\snum'_2(e_1), \rnum'_2(e_1))$.}
\begin{itemize}
...
Without loss of generality, we can assume that there are no transitions out of $F$ in $\aut_{e_1}$. If this is not the case, then a new state $q_f$ can be introduced to transform $\aut_{e_1}$ into a CEFA satisfying this constraint, where $q_f$ is the unique accepting state.\footnote{\hdh{Note that if any initial states are in $F$, a new state $q_I$ without transitions out can be introduced and add to $F$, to accept empty string. Then we can remove all original initial states out of $F$.}} 
...
Since these two types of approximation influence each other, we separate them apart and run them parallelly.
}
...
\hdh{The naive approach of finding satisfied string models is to search the accepted words on CEFAs based on the integer models of vectors, which causes an exponential blow-up of searching space. Specifically, } recall that in Section~\ref{sec:algorithm}, the satisfiability of the original RECL constraint is reduced to the satisfiability of the existential LIA formula
\[
\begin{array}{l}
...
each time when a transition $(q, a, q', \myvec{v})$ in $\cB_i$ is traversed, $\nvec_{i, \myvec{v}}$ is decremented. Note that initially $\nvec_{i, \myvec{v}}$ is set to $\amodel(\anivarz_{i, \myvec{v}})$. The searching process terminates when $\nvec_{i, \myvec{v}}$ becomes $0$ for every $\myvec{v}$. We record the characters in the traversed transitions and obtain a string for the string variable $x_i$ when the searching process terminates. As a result, we obtain a model of $\varphi$ in the end. \hdh{In the worst case, we need search O($\prod_{i=0}^{n} \amodel(\anivarz_{i, \myvec{v}})$) times to find an accepted word.}
