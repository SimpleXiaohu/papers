...
At first, we show how to construct a CEFA from a regex where counting operators may occur. \hdh{Glushkov’s construction~\cite{Gluskov61} and its variants~\cite{redos_lenka,HS+23,GKM23} convert a regex to a $\epsilon$-free automaton. In contrast, Thompson's construction~\cite{Thompson1968} results in an automaton with $\epsilon$-transitions but the algorithm is simpler and straightforward. }Our approach is based on Thompson's construction~\cite{Thompson1968} \hdh{and we remove $\epsilon$-transitions on the fly (See union at Section \ref{sec:automaton} and construction of counting operators below)}. Let us start with register-representable regexes defined in the sequel. 
...
\medskip
\noindent
\emph{Step 3 (Computing Parikh images \hdh{and derive a solution}).} For each $i \in [n]$, we compute an existential LIA formula $\psi_{\cC_i}(\fZ_{\costset_{\cC_i}})$ from $\cC_i$ to represent $\parikh(\cC_i)$. \hdh{Subsequently, we can employ established SMT solvers to determine the satisfiability of the formula and derive a solution if it is satisfiable. The integer variable solutions are directly extracted from the partial model of the formula. The string variable solutions are obtained through indirect means, whose detailed methodology is discussed in Section \ref{sec-opt-sol-gen}. The final Parikh images are as follow :}
%
\[
\begin{array}{l}
...
\hdh{
\subsection{Complexity Analysis of the $\reclsat$ Problem}
The time complexity of regular expression equivalence with complement has been established as nonelementary~\cite{complementary_complexity_1974}. This directly implies that the $\reclsat$ problem exhibits a nonelementary lower bound. We now turn our attention to analyzing the complexity of the $\reclsat$ problem in the absence of complement operations.

\begin{lemma}\label{lemma:regex2cefa}
    Register-representable regex without complement operation can be converted to CEFA in PSPACE.
\end{lemma}

\begin{proof}
    We prove this lemma through structural induction on regular expression $e$. For the base case—where the regular expression consists of a single character, $\epsilon$, or $\emptyset$—the lemma holds trivially.
    For the inductive cases, assuming the lemma holds for regular expressions $e_1$ and $e_2$:
    When $e = e_1\concat e_2$ or $e = e_1+e_2$, the concatenation or union operations described in Section \ref{sec:automaton} produce a CEFA of size $O(|e_1|^{k_1} + |e_2|^{k_2})=O(|e|^{\max(k_1, k_2)})$.
    For $e = e_1\cap e_2$, the intersection operation detailed in Section \ref{sec:automaton} yields a CEFA of size $O(|e_1|^{k_1}\cdot|e_2|^{k_2})=O(|e|^{k_1+k_2})$.
    In the case of $e=e_1^{\{m,n\}}$, our construction in Section \ref{subsec:regex2cefa} avoids direct unfolding of counting operators, resulting in a CEFA of size $O(|e_1|^{k_1})=O(|e|^{k_1})$.
    Finally, for $e=(e_1)$ and $e=e^*$, our application of Thompson's construction~\cite{Thompson1968} produces a CEFA of size $O(|e_1|^{k_1})=O(|e|^{k_1})$.
\end{proof}

\begin{theorem}\label{theorem_pspace}
    If all regexes are register-representable and complement operations are excluded, $\reclsat$ can be solved in PSPACE-complete.
\end{theorem}
\begin{proof}
    Based on Lemma \ref{lemma:regex2cefa}, our construction yields a CEFA of polynomial size for each register-representable regex without complement operation. Furthermore, for each string variable's length function, a CEFA of constant size is constructed in Section \ref{subsec:regex2cefa}.
    Consequently,  the construction outlined in Section \ref{subsec:regex2cefa} requires nondeterministic polynomial size. By Proposition \ref{prop-cefadec}, we can demonstrate that the procedure described in Section \ref{subsec:cefadec} likewise requires nondeterministic polynomial size. Therefore, the complexity of the complete decision procedure for solving the restricted $\reclsat$ problem is NPSPACE-complete, which yields PSPACE-complete by Savitch's theorem~\cite{complexity-book}.
\end{proof}

\begin{theorem}
    $\reclsat$ can be solved in EXPSPACE when complement operations are excluded.
\end{theorem}

\begin{proof}
    In Section \ref{subsec:regex2cefa}, unfolding non-register-representable occurrences of counting operators leads to an exponential growth in expression length. For each non-register-representable regular expression of length $l$, the unfolding process generates a register-representable regular expression of length $O(m^n\cdot l)=O((2^{|l|})^{|l|}\cdot l) = O(2^{|l|})$, where $m$ represents the maximum counting bound and $n$ denotes the maximum depth of nested counting operators. By Theorem \ref{theorem_pspace}, the complexity of the complete decision procedure for solving the $\reclsat$ problem without complement operations is EXPSPACE.
\end{proof}
}
