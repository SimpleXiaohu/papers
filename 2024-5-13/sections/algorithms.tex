...
We utilize some off-the-shelf SMT solver (e.g. Z3) to solve the satisfiability of the LIA formula in Equation~(\ref{eq:parikh_image}). If the solver reports ``sat'' and returns a solution $\amodel$, then we go to Step 5. Otherwise, the $\cefadec$ problem has no solution. 
...
The construction of $\amodel'(x_i)$ is actually to solve the reachability problems in a finite transition system where 
\begin{itemize}
...
If a path from the initial state to the final state is found, then the path gives us a desired string $\amodel'(x_i)$.
}
...
The time complexity of regular expression equivalence with complement has been established as nonelementary~\cite{complementary_complexity_1974}. This directly implies that the $\reclsat$ problem exhibits a nonelementary lower bound. We now turn our attention to analyzing the complexity of the $\reclsat$ problem in the absence of complement operations (no occurrence of $\Bar{e}$ and $e_1 \backslash e_2$).

\begin{lemma}\label{lemma:regex2cefa}
    Register-representable regex without complement operation can be converted to CEFA in EXPSPACE.
\end{lemma}
...
\begin{proof}
    ...
    When $e = e_1\concat e_2$ or $e = e_1+e_2$, the concatenation or union operations described in Section \ref{sec:automaton} produce a CEFA of size $O(2^{|e_1|} + 2^{|e_2|}) = O(2^{\max(|e_1|, |e_2|)}) = O(2^{|e|})$.
    For $e = e_1\cap e_2$, the intersection operation detailed in Section \ref{sec:automaton} yields a CEFA of size $O(2^{|e_1|}\cdot 2^{|e_2|})=O(2^{|e_1| + |e_2|})=O(2^{|e|})$.
    In the case of $e=e_1^{\{m,n\}}$, our construction in Section \ref{subsec:regex2cefa} avoids direct unfolding of counting operators, resulting in a CEFA of size $O(2^{|e_1|}) = O(2^{|e|})$.
    Finally, for $e=(e_1)$ and $e=e^*$, our application of Thompson's construction~\cite{Thompson1968} produces a CEFA of size $O(2^{|e_1|}) = O(2^{|e|})$.
\end{proof}
